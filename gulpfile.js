...
const rimraf = require("rimraf");//清空文件夹
...
gulp.task("dev", [ "clean" ], cb => {
    const config = require("./webpack.config.dev.js");//指定需要记载的js文件
    const complier = webpack(config);

    new WebpackDevServer(complier, {//配置参数.方便调试
        publicPath: config.output.publicPath,
        proxy: config.devServer.proxy,//请求数据代理
        hot: true,
